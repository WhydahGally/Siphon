...
During `siphon add --download`, `siphon sync`, and `siphon sync-failed`, the CLI SHALL print a numbered list of all items planned for download before any download begins. As each item completes, the CLI SHALL print a result block for that item (all lines for the item printed atomically, with no interleaving from other concurrent threads). After all items complete, the CLI SHALL print a summary line.

#### Scenario: Planned items list printed before download starts
- **WHEN** `_sync_one` (or the `add --download` path) has filtered entries and is about to dispatch
