...
Each `JobItem` within a `DownloadJob` SHALL have a `state` field that follows this transition graph: `pending → downloading → done` or `pending → downloading → failed`. No other transitions are valid. State transitions SHALL be applied by the download worker thread and SHALL notify the `JobStore`'s SSE subscribers via an `asyncio.Queue`.
