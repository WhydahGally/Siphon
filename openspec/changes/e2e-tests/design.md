...
| Secret | Purpose |
|---|---|
| `SIPHON_E2E_PLAYLIST_URL` | A small public playlist (~5 items) owned by the repo maintainer |
| `SIPHON_E2E_VIDEO_URL` | A single short video (< 60s) for fast download tests |
| `SIPHON_E2E_KNOWN_TRACK_URL` | A video of a well-known song where MusicBrainz lookup is reliable |
| `SIPHON_E2E_MB_USER_AGENT` | MusicBrainz-compliant user agent string |
