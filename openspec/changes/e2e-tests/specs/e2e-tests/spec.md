...
- **WHEN** `DELETE /jobs/{id}` is called while a download job is in `downloading` state
- **THEN** `GET /jobs` shows the job in `cancelled` state within 10 seconds
...
The e2e suite SHALL verify that the correct rename tier is stored per item.

#### Scenario: yt_metadata tier used when artist/track tags present
- **WHEN** a video has YouTube metadata tags with artist and track information
- **THEN** `GET /playlists/{id}/items` shows `rename_tier=yt_metadata` for that item
...
#### Scenario: yt_title tier used as fallback
- **WHEN** no artist/track metadata is available and MusicBrainz returns no confident match
- **THEN** the item's `rename_tier` is `yt_title`
...
#### Scenario: MusicBrainz lookup respects rate limit
- **WHEN** the e2e suite runs multiple MusicBrainz lookups in sequence
- **THEN** no HTTP 429 response is observed from the MusicBrainz API
...
- **WHEN** `SIPHON_E2E_PLAYLIST_URL` is not set in the environment
