...
`JobItem` SHALL also have mutable `renamed_to` and `rename_tier` fields. These fields are initially set during download (by the auto-renamer if active, otherwise by the passthrough rename). After download completes (`done` state), `renamed_to` and `rename_tier` MAY be updated by the manual rename endpoint without changing the item's `state`.
